...
def parse_args():
    ...
    parser.add_argument('--tfr_start_decay_epoch', type=int, default=15, help='The epoch that teacher forcing ratio become decreasing')
    parser.add_argument('--tfr_decay_step', type=float, default=0.015, help='The decay step size of teacher forcing ratio (0 ~ 1)')
    ...
    parser.add_argument("--c_dim", type=int  , default=7, help="dimensionality of the condition")
...
class kl_annealing():
    def __init__(self, args):
        super().__init__()
        self.L = self.frange_cycle_linear(args)
        self.idx = 0

    def frange_cycle_linear(self, args):
        L = np.ones(args.niter)
        n_cycle = args.kl_anneal_cycle if args.kl_anneal_cyclical else 1
        period = args.niter/n_cycle
        step = 1 / (period*args.kl_anneal_ratio) # linear schedule
...
def main():
    ...
    for epoch in range(start_epoch, start_epoch + niter):
        ...
        for _ in range(args.epoch_size):
            ...
            # (B, N, C, Rows, Cols) -> (N, B, C, Rows, Cols)
            # (B, N, P) -> (N, B, P)
        ...
        if epoch >= args.tfr_start_decay_epoch:
            ...
            args.tfr = max(args.tfr_lower_bound, args.tfr-args.tfr_decay_step)
        ...
        kl_losses.append(epoch_kld / args.epoch_size)
    ...
    plot_kl_curve([i for i in range(start_epoch, start_epoch+niter, 1)], kl_losses, kl_betas, tfrs, "KL loss curve", args.log_dir)
    plot_psnr_curve([i for i in range(start_epoch, start_epoch+niter, 5)], psnrs, "PSNR curve", args.log_dir)
