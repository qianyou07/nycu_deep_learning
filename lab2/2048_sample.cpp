...
#define POP_TWO_PROB 0.9
#define POP_FOUR_PROB 0.1
...
void plot_result(std::vector<double> &scores, std::vector<int> &episodes){
	plt::backend("Agg");
...
public:
	...
	state select_best_move(const board& b) const {
		...
		for (state* move = after; move != after + 4; move++) {
			if (move->assign(b)) {
				// TODO
				// check every possible empty tile for new tile in s'
				float value = 0;
				unsigned int numOfEmpty = 0;
				for(int i=0;i<16;i++){
					if(move->after_state().at(i) == 0){
						board board_pop_two = move->after_state(), board_pop_four = move->after_state();
						board_pop_two.set(i, 1);
						board_pop_four.set(i, 2);
						value += POP_TWO_PROB * estimate(board_pop_two) + POP_FOUR_PROB * estimate(board_pop_four);
						numOfEmpty++;
					}
				}
				value /= numOfEmpty;
	...
	void update_episode(std::vector<state>& path, float alpha = 0.1) const {
		...
		for(path.pop_back();path.size();path.pop_back()){
			state &s = path.back();
			next_value = update(s.before_state(), alpha * (s.reward() + next_value - estimate(s.before_state())));
		}
	}
...
int main(int argc, const char* argv[]) {
    ...
    tdl.add_feature(new pattern({ 2, 3, 5, 6, 8, 9 }));
	...
	for (size_t n = 1; n <= total; n++) {
		...
		sum_score += score;
		if(n%1000 == 0){
			scores.push_back(sum_score / 1000.0);
			episodes.push_back(n);
			sum_score = 0;
		}
		path.clear();
	}

	plot_result(scores, episodes);
